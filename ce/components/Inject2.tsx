...
import {
  ...
  fetchOrCreateLessonByMaterialId,
  fetchSampleResponses
} from "~/redux/features/lessons"
import { fetchDeviceId } from "~/utils/fetchDeviceId"
import { SelectLang } from "~components/SelectLang"
import { useLocale } from "~hooks/locale"
import {
  createVocabs,
  fetchCurrentOpenedMaterial,
  fetchSummaries,
  fetchVocabs
...
import {
  fetchSavedParaphrases,
  fetchSavedVocab,
  saveVocab
} from "~redux/features/note"
import { clearSubscriptionForm, toggleDisable } from "~redux/features/ui"
...
function highlightSelectedText() {
  ...
  if (selectedText !== "") {
    ...
    const occurrences = document.body.innerText
      .split(/[.!?]/)
      .filter((sentence) =>
...
function App({
  ...
  const shouldShowSubscriptionForm = useAppSelector((state) => {
    return (
      !state.payment.isValidSubscription && state.ui.shouldShowSubscriptionForm
    )
  })
  ...
  const currentOpenedMaterial = useAppSelector((state) => {
    return state.materials.currentOpenedMaterial
  })
  const currentLesson = useAppSelector((state) =>
    state.lesson.lessons.find(
      (lesson) => lesson.materialId === currentOpenedMaterial?.id
    )
  )

  const messages = useAppSelector((state) =>
    currentLesson ? state.message.messageMap[currentLesson.id] || [] : []
  )
  const vocabs = useAppSelector((state) =>
    currentOpenedMaterial
      ? state.materials.vocabs[currentOpenedMaterial.id] || []
      : []
  )
  const summaries = useAppSelector((state) =>
    currentOpenedMaterial
      ? state.materials.summaries[currentOpenedMaterial.id] || []
      : []
  )

  const isCreatingMessage = useAppSelector(
    (state) => state.message.isCreatingMessage
  )
  const isFetchingVocabs = useAppSelector(
    (state) => state.materials.isFetchingVocabs
  )
  const isFetchingSummary = useAppSelector(
    (state) => state.materials.isFetchingSummary
  )
  const isCreatingVocabs = useAppSelector(
    (state) => state.materials.isCreatingVocabs
  )

  const errorSigninMessage = useAppSelector(
    (state) => state.auth.errorSigninMessage
  )
  const errorSignupMessage = useAppSelector(
    (state) => state.auth.errorSignupMessage
  )
  const errorCreateAIMessage = useAppSelector(
    (state) => state.message.errorCreateAIMessage
  )
  const errorFetchCurrentOpenedMaterial = useAppSelector(
    (state) => state.materials.errorFetchCurrentOpenedMaterial
  )
  const statusCreatingVocabs = useAppSelector((state) =>
    currentOpenedMaterial
      ? state.materials.statusCreatingVocabs[currentOpenedMaterial.id]
      : null
  )
  ...
  async function setMpTrackingIdToUser() {
    ...
    if (
      currentUser &&
      (!currentUser.mpTrackingId || currentUser.mpTrackingId !== deviceId)
    ) {
  ...
  useEffect(() => {
    ...
    if (
      !isCreatingMessage &&
      currentLesson &&
      !currentLesson.initMessage &&
      messages.length === 0
    ) {
      dispatch(
        createAIMessage({
          message: "Ask me a question about the news.",
  ...
  return (
    <Box
      ...
      {errorFetchCurrentOpenedMaterial && (
      ...
      {!isLoading && !currentUser && (
        <Stack
          ...
          {errorSigninMessage && (
            <Alert severity="error">{errorSigninMessage}</Alert>
          )}
          {errorSignupMessage && (
            <Alert severity="error">{errorSignupMessage}</Alert>
          )}
          <ButtonGoogleAuth size="large" onClick={handleSignup} isSignup />
...
export function Inject() {
  ...
  const uiDisabled = useAppSelector((state) => {
    return state.ui.disabled
  })
  ...
  if (
    uiDisabled ||
    isRoot ||
    blaklist.some((url) => window.location.href.includes(url))
  ) {
    return null
  }
...
function VocabMode({ vocabs, onSaveVocab, statusCreatingVocabs }) {
  const dispatch = useAppDispatch()
  const materialId = useAppSelector(
    (state) => state.materials.currentOpenedMaterial?.id
  )
  ...
  return (
    <Box mt={2}>
      ...
      <Stack direction="row" spacing={0.5} py={1} sx={{ position: "relative" }}>
          ...
          return (
            <Tooltip
              ...
              <Chip
                ...
                color={
                  levelsFilter.includes(level) ||
                  (levelsFilter.length === 0 && level === "all")
                    ? "primary"
                    : "default"
                }
              />
      ...
      <ListVocab
        ...
        isLoading={!vocabs || vocabs.length === 0}
    ...
    <Stack>
      ...
      {open && (
        <Stack
          ...
          <Box sx={{ paddingY: 1, borderTop: "1px solid #f4f4f4" }}>
            <Typography onClick={handleSignout} fontSize="h6">
...
// ]
function Tabs({ onClickButton, selectedMode }) {
  return (
    ...
    <ButtonGroup
      ...
      <Button
        ...
        <Typography
          variant="body1"
          component="span"
          sx={{ marginLeft: 0.5, fontWeight: 600 }}>
      ...
      <Button
        ...
        <Typography
          variant="body2"
          component="span"
          sx={{ marginLeft: 0.5, fontWeight: 600, fontSize: 12 }}>
