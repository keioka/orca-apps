...
const chatPolly: ValidatedEventAPIGatewayProxyEvent<typeof schema> = async (event) => {
  ...
  const params = {
    ...
    OutputFormat: 'mp3',
    OutputS3BucketName: 'orca-chat-audio',
    OutputS3KeyPrefix: event.body.messageId + "/",
    VoiceId: voiceId, // Ruth
  };

  try {
    const response: AWS.Polly.StartSpeechSynthesisTaskOutput = await new Promise(
      (resolve, reject) => {
...
export const main = middyfy(chatPolly);
